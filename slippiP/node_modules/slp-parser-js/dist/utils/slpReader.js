...
var _ubjson = require("@shelacek/ubjson");
...
function getRef(input) {
  switch (input.source) {
    case 'file':
      const fd = _fs.default.openSync(input.filePath, "r");
...
function readRef(ref, buffer, offset, length, position) {
  switch (ref.source) {
    case 'file':
      return _fs.default.readSync(ref.fileDescriptor, buffer, offset, length, position);
...
function getLenRef(ref) {
  switch (ref.source) {
    case 'file':
      const fileStats = _fs.default.fstatSync(ref.fileDescriptor) || {};
...
function closeSlpFile(file) {
  ...
  _fs.default.closeSync(file.ref.fileDescriptor);
...
function iterateEvents(slpFile, callback, startPos = null) {
  ...
  const commandPayloadBuffers = _lodash.default.mapValues(slpFile.messageSizes, size => new Uint8Array(size + 1));
  ...
  switch (command) {
    case Commands.GAME_START:
      return {
        ...
        players: _lodash.default.map([0, 1, 2, 3], playerIndex => {
          ...
          const nametag = (0, _fullwidth.toHalfwidth)(_iconvLite.default.decode(nametagBuf, 'Shift_JIS').split('\0').shift());
...
function getMetadata(slpFile) {
  ...
  try {
    metadata = (0, _ubjson.decode)(buffer);
  } catch (ex) {
    // Do nothing
    console.log(ex);
  } // $FlowFixMe
