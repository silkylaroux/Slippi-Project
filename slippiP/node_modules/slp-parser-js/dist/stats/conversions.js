...
exports.generateConversions = generateConversions;
...
function generateConversions(game) {
  const conversions = [];
  ...
  let state = initialState; // Iterates the frames in order in order to compute conversions
  ...
  (0, _common.iterateFramesInOrder)(game, () => {
    ...
    const opntInControl = (0, _common.isInControl)(opponentFrame.actionStateId);
    const opntDidLoseStock = (0, _common.didLoseStock)(opponentFrame, prevOpponentFrame); // Update percent if opponent didn't lose stock
    ...
    if (state.resetCounter > _common.Timers.PUNISH_RESET_FRAMES) {
      shouldTerminate = true;
...
function addOpeningTypeToConversions(game, conversions) {
  const conversionsByPlayerIndex = _lodash.default.groupBy(conversions, 'playerIndex');

  const keyedConversions = _lodash.default.mapValues(conversionsByPlayerIndex, playerConversions => _lodash.default.keyBy(playerConversions, 'startFrame'));

  const initialState = {
    opponentConversion: null
  }; // Only really doing assignment here for flow

  let state = initialState; // console.log(punishesByPlayerIndex);
  ...
  (0, _common.iterateFramesInOrder)(game, () => {
    ...
    const frameNum = frame.frame; // Clear opponent conversion if it ended this frame

    if (_lodash.default.get(state, ['opponentConversion', 'endFrame']) === frameNum) {
      state.opponentConversion = null;
    } // Get opponent conversion. Add to state if exists for this frame


    const opponentConversion = _lodash.default.get(keyedConversions, [indices.opponentIndex, frameNum]);

    if (opponentConversion) {
      state.opponentConversion = opponentConversion;
    }

    const playerConversion = _lodash.default.get(keyedConversions, [indices.playerIndex, frameNum]);
